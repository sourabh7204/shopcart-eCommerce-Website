...
const AddToCartButton = ({
  ...
  const handleAdd = useCallback(() => {
    ...
    if ((product?.stock as number) > itemCount) {
      ...
      toast.error("Stock limit reached ⚠️");
    }
  }, [product, itemCount, addItem, isOutOfStock]);

  const handleRemove = useCallback(() => {
    removeItem(product._id);
    haptic();
    if (itemCount === 1) toast("Item removed 🗑️");
  }, [product._id, removeItem, itemCount]);

  // 🎨 Wishlist variant (compact style with + / – controls)
  if (variant === "wishlist") {
    return (
        ...
        <AnimatePresence mode="wait">
          {itemCount > 0 ? (
            <motion.div
              ...
              className="flex items-center gap-2 px-2 py-1 bg-gradient-to-r from-shop_light_green/10 to-shop_dark_green/10 border border-shop_dark_green/20 rounded-full"
            >
              {/* - Button */}
              <motion.button
                whileTap={{ scale: 0.9 }}
                onClick={handleRemove}
                className="h-6 w-6 rounded-full bg-white border border-shop_dark_green/40 flex items-center justify-center hover:bg-shop_dark_green/10 transition"
              >
                <Minus className="w-3.5 h-3.5 text-shop_dark_green" />
              </motion.button>
              ...
              <motion.span
                ...
                animate={{ scale: 1, opacity: 1 }}
                transition={{ type: "spring", stiffness: 300 }}
                className="text-xs font-semibold min-w-[20px] text-center text-shop_dark_green"
              >
                {itemCount}
              </motion.span>
              ...
              <motion.button
                ...
                onClick={handleAdd}
                disabled={(product?.stock as number) <= itemCount}
                className="h-6 w-6 rounded-full bg-shop_dark_green flex items-center justify-center text-white hover:bg-shop_dark_green/90 transition disabled:opacity-40"
              >
            ...
            <motion.button
              ...
              disabled={isOutOfStock}
              className={cn(
                "flex items-center gap-1 px-3 py-1.5 rounded-full text-xs font-semibold transition-all duration-200 shadow-sm hover:shadow-md disabled:opacity-50",
                isOutOfStock
                  ? "bg-gray-200 text-gray-500 cursor-not-allowed"
                  : "bg-gradient-to-r from-shop_dark_green to-shop_light_green text-white hover:brightness-110",
                className
              )}
            >
  ...
  // 🛒 Default variant (for product page)
  return (
    <Button
      onClick={handleAdd}
      disabled={isOutOfStock}
      className={cn(
        "w-full flex items-center justify-center gap-2 bg-gradient-to-r from-shop_dark_green to-shop_light_green text-white text-sm font-semibold rounded-xl py-3 shadow-md hover:shadow-xl transition-all duration-300 hover:brightness-110 disabled:opacity-50",
        className
      )}
    >
      {isOutOfStock ? (
